...
class AuthService:
    ...
    def authenticate_user(
        ...
        if company_name and company_name.strip():
            users = query_all(
                "SELECT * FROM users WHERE (username = %s OR email = %s) AND LOWER(company_name) = LOWER(%s)",
                (ident, ident, company_name.strip())
            )
        else:
            users = query_all(
                "SELECT * FROM users WHERE username = %s OR email = %s ORDER BY (username = %s) DESC, id DESC",
                (ident, ident, ident)
            )
    ...
    def reset_password(cls, identifier: str, new_password: str, company_name: Optional[str] = None) -> bool:
        ...
        if company_name and company_name.strip():
            ...
            # Prioritize unique username match
            user = query_one("SELECT * FROM users WHERE username = %s", (ident,))
            if not user:
                users_by_email = query_all("SELECT * FROM users WHERE email = %s", (ident,))
